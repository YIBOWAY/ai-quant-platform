...
def test_prediction_market_timeseries_artifact_404_uses_standard_detail(
    ...
    missing = client.get(
        f"/api/prediction-market/timeseries-backtest/{run_id}/artifacts/missing.png"
    )

    assert missing.status_code == 404
    detail = missing.json()["detail"]
    assert detail["code"] == "not_found"
    assert detail["resource"] == "prediction_market_timeseries_artifact"
    assert detail["id"] == f"{run_id}/missing.png"
