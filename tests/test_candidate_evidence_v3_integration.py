...
class _FakeGateway:
    ...
    run_event_logs: dict[str, tuple[dict[str, object], ...]]
    ...
    def run_events(self, run_id: str) -> tuple[dict[str, object], ...]:
        return tuple(dict(event) for event in self.run_event_logs[run_id])

    def run_status(self, run_id: str) -> dict[str, object]:
        return dict(self.run_statuses[run_id])
...
class _Scenario:
    ...
    fork_hermes_session_id: str
    fork_resolved_source_session_id: str
    gate1_id: str
    gate1_hqa_ref: str
    gate3_id: str
    gate3_hqa_ref: str
...
class _ProvisionPort:
    def __init__(self, *, resolved_source_session_id: str | None = None) -> None:
        self.resolved_source_session_id = resolved_source_session_id
        ...
        *,
        ...
        return ManagedSessionProvisionReceipt(
            ...
            resolved_source_session_id=(
                self.resolved_source_session_id or source_session_id
            ),
...
def _complete_command(
    ...
    *,
    ...
    run_id: str,
    label: str,
    terminal_state: Literal["succeeded", "cancelled"] = "succeeded",
) -> str:
    ...
    terminal_method = (
        ...
        else ledger.mark_cancelled
    )
    terminal = terminal_method(
    ...
    return str(terminal.command_id)
...
def _run_event(
    ...
    **values: object,
    ...
    return {
        ...
        "run_id": run_id,
        "seq": seq,
        "timestamp": f"2026-07-24T01:00:{seq:02d}.000000Z",
        **values,
    }


def _complete_control_outcome(
    environment: _DatabaseEnvironment,
    *,
    workspace_id: str,
    ...
    created = HermesCommandLedger(environment.settings).create_command(
    ...
    outcome = RunControlOutcomeAuthority(environment.settings).finalize(
    ...
    assert outcome.command_state == "succeeded"
    return str(created.command_id)
...
def _seed_scenario(
    ...
    *,
    ...
    fork_resolved_source_session_id = (
        f"discord-compression-tip-{suffix}"
        if compressed_fork
        else source_hermes_session_id
    )
    fork_hermes_session_id = _register_managed_session(
        ...
        resolved_source_session_id=fork_resolved_source_session_id,
    )
    ...
    paper_subject_run_id = f"paper-research-{suffix}"
    paper_subject_command_id = _complete_command(
        environment,
        admission_id=admission_id,
        platform_session_id=web_platform_session_id,
        hermes_session_id=web_hermes_session_id,
        run_id=paper_subject_run_id,
    ...
    stopped_source_command_id = _complete_command(
        ...
        label=f"stopped-{suffix}",
        terminal_state="cancelled",
    )

    approval_challenge_id = f"approval-challenge-{suffix}"
    approval_core_id = f"approval-core-{suffix}"
    approval_command_digest = _digest(f"approval-command:{suffix}")
    approval_expires_at = "2099-01-01T00:00:00.000000Z"
    approval_action = DecideHermesCommandApproval(
        client_action_id=f"approval-control-{suffix}",
        workspace=WorkspaceRef(workspace_id),
        approval_ref=f"approval:{approval_challenge_id}",
        run_ref=f"run:{options_run_id}",
        command_digest=approval_command_digest,
        expected_status="pending",
        expected_expires_at=approval_expires_at,
        decision="allow_once",
    )
    approval_control_command_id = _complete_control_outcome(
        environment,
        workspace_id=workspace_id,
        client_action_id=approval_action.client_action_id,
        action_digest=canonical_action_digest(approval_action),
        action_kind="hermes_command_approval_decide",
        target_run_id=options_run_id,
        external_status="committed",
    )
    stop_action = RequestStop(
        client_action_id=f"stop-control-{suffix}",
        workspace=WorkspaceRef(workspace_id),
        run_ref=f"run:{stopped_run_id}",
        task_ref=None,
        attempt_ref=None,
        platform_job_ref=None,
    )
    stop_control_command_id = _complete_control_outcome(
        environment,
        workspace_id=workspace_id,
        client_action_id=stop_action.client_action_id,
        action_digest=canonical_action_digest(stop_action),
        action_kind="run_stop_request",
        target_run_id=stopped_run_id,
    ...
    subject_attestation_digest = _digest(f"paper-attestation:{suffix}")
    provider_evidence_ref = (
        f"provider-evidence:paper-run-{subject_attestation_digest}"
    )
    hqa_run_ref = f"run:{paper_subject_run_id}"
    final_receipt_digest = _digest(f"final-receipt:{suffix}")
    final_config_digest = _digest(f"final-config:{suffix}")
    final_summary_digest = _digest(f"final-summary:{suffix}")
    final_report_digest = _digest(f"final-report:{suffix}")
    ...
    paper.register_challenge(
        RegisterPaperGateChallenge(
            ...
            final_backtest_config_ref=f"/evidence/{suffix}/config.json",
            final_backtest_config_digest=final_config_digest,
            final_backtest_summary_ref=f"/evidence/{suffix}/summary.json",
            final_backtest_summary_digest=final_summary_digest,
            final_backtest_report_ref=f"/evidence/{suffix}/report.json",
    ...
    completion_evidence = {
        ...
        "plan_version": 1,
        "plan_digest": _digest(f"paper-plan:{suffix}"),
        "plan_confirmation_note_digest": _digest(
            f"paper-plan-confirmation:{suffix}"
        ),
        "attempt_ref": research_attempt_ref,
    ...
    all_run_ids = (
        ...
        paper_gate2_run_id,
        paper_gate3_run_id,
        paper_subject_run_id,
        stopped_run_id,
    )
    run_event_logs = {
        run_id: (
            _run_event(run_id, 1, "run.started"),
            _run_event(run_id, 2, "run.completed"),
        )
        for run_id in all_run_ids
    }
    run_event_logs[options_run_id] = (
        _run_event(
            options_run_id,
            1,
            "approval.request",
            challenge_id=approval_challenge_id,
            approval_id=approval_core_id,
            action_digest=approval_command_digest,
        ...
        _run_event(options_run_id, 5, "run.completed"),
    )
    run_event_logs[stopped_run_id] = (
        _run_event(stopped_run_id, 1, "run.started"),
        _run_event(stopped_run_id, 2, "run.stop_requested"),
        _run_event(
            stopped_run_id,
            3,
            "run.cancelled",
    ...
    run_statuses = {
        run_id: {
            "object": "hermes.run",
            "run_id": run_id,
            "session_id": web_hermes_session_id,
            "status": "succeeded",
        }
        for run_id in all_run_ids
    }
    run_statuses[stopped_run_id] = {
        "object": "hermes.run",
        "run_id": stopped_run_id,
        "session_id": web_hermes_session_id,
        "status": "stopped",
    ...
    gateway = _FakeGateway(
        ...
        details={
            fork_hermes_session_id: {
                ...
                "parent_session_id": fork_resolved_source_session_id,
            }
        },
        run_event_logs=run_event_logs,
    ...
    return _Scenario(
        ...
        web_command_ids=web_command_ids,
        fork_hermes_session_id=fork_hermes_session_id,
        fork_resolved_source_session_id=fork_resolved_source_session_id,
        gate1_id=gate1_id,
...
def _insert_forged_command_with_mismatched_run(
    ...
    forged_run_id = f"forged-run-{command_id.hex[:12]}"
    with environment.runtime.connect() as conn:
        conn.execute(
            """
    ...
    scenario.gateway.run_event_logs[forged_run_id] = (
        _run_event(forged_run_id, 1, "run.started"),
        _run_event(forged_run_id, 2, "run.completed"),
    )
    scenario.gateway.run_statuses[forged_run_id] = {
        "object": "hermes.run",
        "run_id": forged_run_id,
        "session_id": scenario.web_hermes_session_id,
        "status": "succeeded",
    }
...
def test_candidate_evidence_binds_selected_and_compression_resolved_fork_lineage(
    ...
    scenario = _seed_scenario(database_environment, compressed_fork=True)
    _capture_valid_restart(scenario)

    verified = scenario.authority.verify_and_store(scenario.refs)

    fork_flow = verified.facts["flows"]["exact_message_fork"]  # type: ignore[index]
    assert fork_flow["source_hermes_session_id"] != (
        fork_flow["resolved_source_session_id"]
    )
    assert (
        fork_flow["resolved_source_session_id"]
        == scenario.fork_resolved_source_session_id
    )
    assert fork_flow["fork_point"] == "message:2"


def test_candidate_evidence_rejects_child_parent_drift_from_resolved_lineage(
    database_environment: _DatabaseEnvironment,
) -> None:
    scenario = _seed_scenario(database_environment, compressed_fork=True)
    _capture_valid_restart(scenario)
    scenario.gateway.details[scenario.fork_hermes_session_id][
        "parent_session_id"
    ] = "discord-unexpected-tip"

    with pytest.raises(CandidateEvidenceV3Error) as rejected:
        scenario.authority.verify_and_store(scenario.refs)

    assert rejected.value.code == "candidate_evidence_fork_mismatch"
