...
def _completion_request(
    *,
    ...
    evidence = {
        ...
        "provider_evidence_ref": "provider-evidence:futu-paper-final",
...
def _test_url() -> str:
    ...
    if not dbname or not (dbname.endswith("_tmp") or "test" in dbname):
        pytest.fail("QS_TEST_DATABASE_URL must point at a throwaway database")
    return url


def _admin_test_url() -> str:
    url = os.environ.get("QS_TEST_DATABASE_ADMIN_URL")
    if not url:
        pytest.skip("set QS_TEST_DATABASE_ADMIN_URL for destructive readiness tests")
    dbname = psycopg.conninfo.conninfo_to_dict(url).get("dbname")
    if not dbname or not (dbname.endswith("_tmp") or "test" in dbname):
    ...
    with Database(_test_url(), connect_timeout=1).connect() as conn:
        ...
        conn.execute(
            ...
            VALUES (
                ...
                'succeeded', 1, 1, clock_timestamp(), %s, %s
            )
            """,
            (
                command_id,
...
def _authority() -> PaperGateAuthority:
    settings = _settings()
    database = Database(_test_url(), connect_timeout=1)
...
def test_registration_requires_exact_ready_platform_managed_session() -> None:
    ...
    with (
        ...
        pytest.raises(
            ...
            match="exact active or succeeded Command",
        ),
    ):
        conn.execute(
            """
