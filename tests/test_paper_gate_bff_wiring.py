...
def test_gate1_source_bff_uses_only_path_and_digest_from_durable_authority(
    ...
    class _SourceAuthority:
        ...
        def get_gate1_source_evidence(
            ...
            *,
            ...
            calls.append((workspace_id, gate_id))
            return expected

    monkeypatch.setattr(workspace_routes, "PaperGateAuthority", _SourceAuthority)
    response = workspace_routes.workspace_gate1_source_evidence(
        WORKSPACE_ID,
        "paper-gate-1",
        _settings(),
    ...
    assert calls == [(WORKSPACE_ID, "paper-gate-1")]
    assert Gate1SourceEvidenceResponse.model_validate(response).model_dump() == expected
            ...
            return [
                {
                    ...
                    "managed_session_ref": "session:managed-paper-session",
                    "source_file_ref": "/tmp/paper-reversal-factor.py",
