...
- Migration 005 now provides the PostgreSQL command/event/outbox/run-link
  ...
  **V7a–V7d M1 ACCEPT** (exact allow_once|deny CAS + hermetic respond_approval release/signal + hermetic Run-scoped stop with §5.5 layered receipt + durable approval projector pending/decided on snapshot/follow spine; no always-allow; ≠ Gate; no Task invention). Next: V7e+ (Gates/results/verticals);
