...
- Migration 005 now provides the PostgreSQL command/event/outbox/run-link
  ...
  **V7a decide + V7b release + V7c stop M1 ACCEPT** (exact allow_once|deny CAS + hermetic respond_approval release/signal + hermetic Run-scoped stop with §5.5 layered receipt; no always-allow; ≠ Gate; no Task invention). Next: V7d+ (durable projector/Gates/results);
