...
> 已交付要点：单一持久账户（默认 100 万）、手动下单（数量/金额、限价按当前模拟价格检查一次；未满足时记为 `unfilled`，不持久挂单）、策略一键再平衡（plan-then-commit 原子性）、账户级冻结开关、Futu 实时快照取价（离线只回退真实最近收盘）、逐持仓报价来源、账户驱动的持仓地图、CLI 定时再平衡、网页与定时任务跨进程串行保护、移动端导航。代码入口见本文第 10 节与 [../../AGENTS.md](../../AGENTS.md) 的「Paper Account」一节。
