...
- `src/quant_system/api/jobs/backtest_jobs.py`：本地 `ThreadPoolExecutor` backtest job runner。`QS_BACKTEST_JOBS_ENABLED=false` 时 `/api/backtests/run` 保持同步 `200`；开启后返回 `202` job state，支持 `GET /api/backtests/jobs/{run_id}` 轮询和 `POST /api/backtests/jobs/{run_id}/cancel` 协作取消。队列不是分布式持久队列，API 重启时会把遗留的 queued/running/cancelling metadata 标记为 failed。
