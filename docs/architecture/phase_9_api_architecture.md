...
- `src/quant_system/api/schemas/*`：集中放置 Pydantic request / response schema，用于 OpenAPI 契约和前后端字段防漂移。2026-06-15 起，`/api/health`、`/api/symbols`、`/api/ohlcv`、`/api/benchmark`、`/api/backtests`、`/api/backtests/{run_id}`、`/api/experiments`、`/api/experiments/{experiment_id}`、`/api/paper`、`/api/paper/{run_id}`、`/api/strategies`、`/api/universes`、`/api/factors`、`/api/factors/runs`、`/api/factors/{run_id}`、`/api/runs/recent`、`/api/market-data/history`、`/api/options/daily-scan/dates`、`/api/options/daily-scan/status`、`/api/agent/candidates`、`/api/agent/candidates/{candidate_id}`、`/api/agent/llm-config`、`/api/options/tools/strategy/templates`、`/api/options/tools/watchlist` 已先挂 `response_model`；其他路由仍需按域逐步补齐。
