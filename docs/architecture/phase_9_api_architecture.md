...
- `src/quant_system/api/bootstrap.py`：构造 `Settings`、解析输出目录、写入 `app.state`，并挂载轻量本地 backtest job runner。
- `src/quant_system/api/dependencies.py`：从 `app.state` 取 settings、输出目录、运行目录和 backtest job runner。
...
- `src/quant_system/api/jobs/backtest_jobs.py`：本地 `ThreadPoolExecutor` backtest job runner。`QS_BACKTEST_JOBS_ENABLED=false` 时 `/api/backtests/run` 保持同步 `200`；开启后返回 `202` job state，支持 `GET /api/backtests/jobs/{run_id}` 轮询和 `POST /api/backtests/jobs/{run_id}/cancel` 协作取消。API 关闭时最多等待 `QS_BACKTEST_JOBS_SHUTDOWN_TIMEOUT_SECONDS`，超时仍未停止的 job 会被标记为 cancelled。队列不是分布式持久队列，API 重启时会把遗留的 queued/running/cancelling metadata 标记为 failed。
...
- 不加重型队列 / 分布式执行：长 backtest 只使用本地进程内 job runner；需要跨进程恢复时显式标记 failed，而不是假装可继续。
