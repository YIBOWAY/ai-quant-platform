...
- 挂单检查与取消：点击交易面板里的「检查挂单」会调用 `POST /api/paper/account/orders/process`，按当前真实纸面价格重新检查待处理限价单，触价则成交并更新账户；待处理限价单行内的「取消」会调用 `POST /api/paper/account/orders/{order_id}/cancel`，移除该挂单并写入 `order_cancelled` 账本事件。当前版本还没有购买力/可卖数量预留、后台定时检查，也不会在停机后用日内高低价回溯补判。
