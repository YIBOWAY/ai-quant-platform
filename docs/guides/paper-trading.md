...
- 挂单检查：点击交易面板里的「检查挂单」会调用 `POST /api/paper/account/orders/process`，按当前真实纸面价格重新检查待处理限价单，触价则成交并更新账户。当前版本还没有取消挂单、购买力/可卖数量预留，也不会在停机后用日内高低价回溯补判。
...
3. 点「提交订单」。成交后账户摘要与[持仓地图](position-map.md)立即更新；未触价的限价单会出现在「待处理限价单」。后续点击「检查挂单」会按当前价格重新撮合。
