...
- [Paper Strategy Sleeves MVP-1](../design/paper_strategy_sleeves_plan.md) — 策略资金段 / signal-only / allocated 分账设计。2026-06-26 已完成后端基础、API contract、daily signal 生成、手动 signal CLI 和 `/paper-trading` sleeves 工作区；自动成交尚未实现。执行状态见 [Paper Strategy Sleeves 执行说明](../execution/paper_strategy_sleeves.md)。
