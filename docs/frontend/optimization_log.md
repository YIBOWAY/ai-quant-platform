...
| 待办项 | 优先级 | 工作量估算 | 阻塞因素 |
|---|---|---|---|
| 因子实验室移除硬编码 sample | P0 | 4-6 小时 | 需确认后端降级策略 |
| 限价单挂单生命周期 | P1 | 8-12 小时 | 需后端持久化 pending orders |
| 期权筛选器结果区重构 | P1 | 3-4 小时 | 无 |
...
### P0（本周内）
- [ ] 因子实验室真实数据优先
- [ ] 回测详情增加 "Metrics" 区域的对比列（策略 vs 基准）

### P1（两周内）
- [ ] Paper Trading 页面用 Tab 切换（Live Account / Historical Replay）
- [ ] 限价单挂单列表组件
- [ ] 期权筛选器结果区重构为紧凑布局
