...
**后端协同任务（本批未做，待后续）**：原清单为因子实验室真实数据源、限价单持久挂单+日内触价成交、回测整股/最小订单约束、期权雷达日终自动任务、研报复现 run_id 持久化。2026-06-15 时，因子实验室真实数据源、回测整股/最小订单约束、研报复现 run_id 持久化、期权雷达调度入口 + 页面任务状态 + 可选启动补跑 + 扫描锁、限价单基础挂单队列 + 手动取消已后续落地；2026-06-16 又补上启动补跑的常规美股整天休市日回退和输入刷新、限价单资金/持仓预留，以及 API 运行期间的限价单后台自动检查。停机期间日内高低价补判仍待处理。
...
| 限价单挂单生命周期 | P1 | 8-12 小时 | 基础 `pending_orders` 队列、页面列表、手动检查、逐单取消、资金/持仓预留和 API 后台自动检查已完成；停机期间日内高低价补判待做 |
