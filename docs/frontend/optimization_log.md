...
- 因子实验室范围卡现在暴露 `start` / `end` / `lookback` / `force_refresh`，并通过 “Send to Backtest / 发送至回测” 链接把当前 `provider`、`universe_id`、`benchmark_symbol`、`start`、`end`、`lookback` 和已登记 `factor_ids` 预填到 `/backtest`；链接只填表，不自动运行回测。
