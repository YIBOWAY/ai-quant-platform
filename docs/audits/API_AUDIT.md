...
> - “后端 API 全部是无契约裸 `dict` 响应” —— **正在分批修复**：2026-06-15 起 `/api/health`、`/api/symbols`、`/api/ohlcv`、`/api/benchmark`、`/api/strategies`、`/api/universes`、`/api/factors`、`/api/factors/runs`、`/api/runs/recent` 已发布 FastAPI `response_model`；当前进度以 [README.md](README.md) 的状态补充为准。
>
