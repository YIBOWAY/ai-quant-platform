...
> - “后端 API 全部是无契约裸 `dict` 响应” —— **正在分批修复**：2026-06-15 起 `/api/health`、`/api/symbols`、`/api/ohlcv`、`/api/benchmark`、`/api/backtests`、`/api/backtests/{run_id}`、`/api/experiments`、`/api/experiments/{experiment_id}`、`/api/paper`、`/api/paper/{run_id}`、`/api/paper/account`、`/api/paper/account/ledger`、`/api/settings`、`/api/strategies`、`/api/universes`、`/api/factors`、`/api/factors/runs`、`/api/factors/{run_id}`、`/api/runs/recent`、`/api/market-data/history`、`/api/options/daily-scan/dates`、`/api/options/daily-scan/status`、`/api/agent/candidates`、`/api/agent/candidates/{candidate_id}`、`/api/agent/llm-config`、`/api/options/tools/strategy/templates`、`/api/options/tools/watchlist` 已发布 FastAPI `response_model`；当前进度以 [README.md](README.md) 的状态补充为准。
>
