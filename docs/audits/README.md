...
`GET /api/backtests`、`GET /api/universes`、`GET /api/factors`、`GET /api/factors/runs`、
...
列表、回测列表、replay paper-run 列表、最近运行活动流、期权雷达日常扫描状态、Agent candidate 列表、Agent LLM 配置探针与 options local tools 增加薄 wrapper response schema。
