...
| `run_d34_worker.sh` | Five-minute D-34 target. Loads the owner backend env, stays idle unless `QS_D34_WORKER_ENABLED=true`, rejects startup migration and runs one recoverable Mandate/research/canary cycle. |
| `install_d34_worker_launchagent.sh` | Check and replay-safely install `com.aiquant.d34-worker`; installing it does not enable D-34 or apply migration 030–032. |
