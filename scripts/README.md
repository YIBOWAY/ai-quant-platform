...
| `run_agent_v02_connector.sh` | Supervised Agent v0.2 connector target; requires a regular, non-symlink, exact-mode-`600` env, rejects startup migration, binds imports to release source, and always starts `supervised_dispatch`. Its `--check` is provider/network/database-free. |
| `install_agent_v02_connector_launchagent.sh` | Run the connector `--check` before any launchd mutation, then render and replay-safely install only the supervised Agent v0.2 connector LaunchAgent. |
