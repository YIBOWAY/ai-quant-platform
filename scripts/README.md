...
| `run_d34_worker.sh` | Five-minute D-34 target. Loads the owner backend env, stays idle unless `QS_D34_WORKER_ENABLED=true`, rejects startup migration and runs one recoverable Mandate/research/canary cycle. Enabled `--check` requires the formal safety authorities plus real versions/Qlib/LLM JSON/Futu/Docker preflight; disabled `--check` remains import-only. |
