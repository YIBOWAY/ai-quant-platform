...
/**
 * Official owner desk — three ledgers wired to the remote book.
 * Not preview-data.ts. Public composer stays off.
 */
...
export function OfficialDesk() {
  ...
  const [ledger, setLedger] = useState<Ledger>("duty");
  ...
  const hero = useMemo(() => {
    if (bookQuery.isError) {
      return "远程账读不到。下面不会用预览假数据顶上。";
    }
    if (verified.length > 0) {
      return `有 ${verified.length} 条已验证候选等你决定挂不挂。`;
    }
    if (hung.length > 0) {
      return `已挂上 ${hung.length} 条。还没有新的已验证候选。`;
    }
    return "研究账空着。派研究或投入材料；缺公式或缺股票池会停下来。";
  }, [bookQuery.isError, hung.length, verified.length]);

  return (
    <div className="dp" data-theme="graphite" data-official-desk="true">
      <nav className="dp-iconrail" aria-label="三本账">
        ...
        <button
          type="button"
          className="dp-navbtn"
          data-current={ledger === "duty" ? "true" : undefined}
          onClick={() => setLedger("duty")}
        >
        ...
        <button
          type="button"
          className="dp-navbtn"
          data-current={ledger === "research" ? "true" : undefined}
          onClick={() => setLedger("research")}
        >
        ...
        <button
          type="button"
          className="dp-navbtn"
          data-current={ledger === "paper" ? "true" : undefined}
          onClick={() => setLedger("paper")}
        >
      ...
      <header className="dp-topstrip">
        ...
        <Link className="dp-btn" href={localizePath("/hermes", locale)}>
          Hermes 工作台
        </Link>
      </header>
      ...
      <main className="dp-main dp-scroll">
        <section className="dp-hero">
          <div className="dp-hero-kicker">
            <span className="dp-caps">值班 · 研究 · 模拟</span>
          ...
          <div className="dp-hero-sub">
            ...
            <span>
              化石仓（不计入效果） <span className="dp-num">{book?.fossil_count ?? fossils.length}</span>
            </span>
              ...
              {ledger !== "research"
                ? (ledger === "paper" ? hung : [...hung, ...verified]).map((item) => (
                    <tr className="dp-row" key={item.candidate_id}>
                      <td>
                        <span className="dp-status" data-kind={item.status === "hung" ? "hung" : "candidate"}>
              ...
              {ledger === "paper" && fossils.length > 0
                ? fossils.map((item) => (
                    <tr className="dp-row" data-dim="true" key={`fossil-${item.candidate_id}`}>
                      ...
                      <td className="wrap">
                        <div className="dp-summary">{item.objective || "预览种子仓，不是每日观察"}</div>
                      </td>
                      <td>—</td>
      ...
      <aside className="dp-rail" data-open="true" aria-label="本地遥控">
        ...
        <div className="dp-transcript dp-scroll">
          <div className="dp-msg" data-role="hermes">
            <div className="head">
              <span className="who">助手</span>
            </div>
            <div className="body">
              派研究和挂仓是两条命令。材料缺公式或缺股票池会失败，不会发明宇宙。聊天网页不写。
            </div>
          </div>
          {requests.slice(0, 8).map((item) => (
            <div className="dp-receipt" key={item.request_id}>
              ...
              <div>
                <div className="rt">{item.status} · {item.request_id}</div>
      ...
      <footer className="dp-footer">
        <span>正式桌面 · 远程账 · 不是 desk-preview 假 blotter</span>
        <span className="grow" />
