...
const copy = {
  en: {
    ...
    safetyUnavailable: "Safety settings unavailable.",
    safetyNotPaperOnly: "Paper-only mode is not confirmed.",
  ...
  zh: {
    ...
    safetyTitle: "安全状态",
    paperOnly: "仅模拟 · 不接触实盘",
    safetyUnavailable: "安全设置不可用。",
    safetyNotPaperOnly: "尚未确认仅模拟模式。",
    unavailable: "不可用",
    frozen: "kill_switch",
    netInflow: "净流入",
    sinceOpen: "开户以来",
          ...
          {historyTab ? (
            ...
            <>
              ...
              <div className="rounded-xl border border-border-subtle bg-bg-surface p-5">
                ...
                <SafetyRow
                  label="paper_trading"
                  tone={safety.paperTrading === true ? "up" : "neutral"}
                  value={formatSafetyValue(safety.paperTrading, text.unavailable)}
                />
                <SafetyRow
                  label="live_trading"
                  tone={safety.liveTrading === true ? "danger" : "neutral"}
                  value={formatSafetyValue(safety.liveTrading, text.unavailable)}
                />
                <SafetyRow
                  label={text.frozen}
                  tone={!accountDown && account.kill_switch ? "danger" : "neutral"}
                  value={
                    accountDown
                      ? text.unavailable
                      : account.kill_switch
                ...
                <p className="mt-3 font-body-sm text-text-secondary">
                  {!safetyAvailable
                    ? text.safetyUnavailable
                    : paperOnly
                      ? text.paperOnly
                      : text.safetyNotPaperOnly}
                </p>
              </div>
            </>
          )}
...
function formatSafetyValue(value: boolean | null, unavailable: string) {
