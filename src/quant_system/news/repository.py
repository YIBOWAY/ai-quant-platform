"""Optional PostgreSQL cache for read-only AI news items.

The live AI HOT public API remains the primary source for aihot. This module is a
best-effort local mirror used to make the read-only news page resilient when the
upstream source is temporarily unavailable, and to store Horizon-ingested items
under ``provider='horizon'``.
"""
...
_HORIZON_CACHE_WARNING = "Using cached Horizon items from the local database."
...
def cache_aihot_items(
    ...
    *,
    ...
    cache_news_items(page, settings=settings, query=query, provider=_PROVIDER)


def cache_news_items(
    page: AiHotItemsPage,
    *,
    settings: Settings,
    query: AiHotItemsCacheQuery,
    ...
    """Best-effort upsert of news items for any provider into the optional DB."""
    ...
    try:
        with database.connect() as conn:
            _upsert_items(conn, page=page, query=query, provider=provider)
    except Exception as exc:  # noqa: BLE001 - cache must not affect live reads
        log.warning("%s cache write skipped: %s", provider, exc)
...
def load_cached_aihot_items(
    *,
    ...
    return load_cached_news_items(
        settings=settings,
        query=query,
        provider=_PROVIDER,
        cache_warning=_CACHE_WARNING,
    )


def load_cached_news_items(
    *,
    settings: Settings,
    query: AiHotItemsCacheQuery,
    provider: str,
    ...
    """Return cached news items for a provider, or ``None`` if unavailable."""
    ...
    if database is None:
        return None
    try:
        where, params = _where_clause(query, provider=provider)
        ...
        log.warning("%s cache read skipped: %s", provider, exc)
        return None
    ...
    warning = cache_warning or (
        _HORIZON_CACHE_WARNING if provider == "horizon" else _CACHE_WARNING
    )
