...
def build_account_performance(
    *,
    ...
    coverage_complete = bool(aligned_dates) and paper_series["status"] == "available"
    coverage_complete = coverage_complete and all(
        benchmark_statuses[symbol] == "available" for symbol in normalized_benchmarks
    )
    coverage_complete = coverage_complete and aligned_dates == primary_dates
...
def _benchmark_status(
    *,
    ...
    if not dates:
        return "unavailable"
    return "available" if dates.issuperset(primary_dates) else "partial"
