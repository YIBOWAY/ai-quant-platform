...
_PAPER_RUN_ATTESTATION_RE = re.compile(r"^paper-run-attestation:([0-9a-f]{64})$")
...
class PaperGateCompletionRecord:
    ...
    def to_operator_dict(self) -> dict[str, object]:
        return {
            ...
            "final_backtest_summary_digest": self.final_backtest_summary_digest,
            "final_backtest_report_ref": self.final_backtest_report_ref,
            "final_backtest_report_digest": self.final_backtest_report_digest,
            "plan_version": self.plan_version,
            "plan_digest": self.plan_digest,
            "plan_confirmation_note_digest": (self.plan_confirmation_note_digest),
        ...
        payload.update(
            {
                ...
                "subject_command_id": (
                    None if self.subject_command_id is None else str(self.subject_command_id)
                ),
                "subject_hermes_run_id": self.subject_hermes_run_id,
                "subject_run_attestation_ref": self.subject_run_attestation_ref,
                "subject_run_attestation_digest": (self.subject_run_attestation_digest),
...
def _optional_absolute_ref(value: object, field: str) -> str | None:
    if value is None:
        return None
    if (
        type(value) is not str
        ...
        raise PaperGateAuthorityValidationError(f"{field} must be a bounded absolute path")
    return value
...
def _optional_row_text(value: object, *, strip: bool = False) -> str | None:
    if value is None:
        return None
    text = str(value)
...
def _record_from_row(row: tuple[object, ...]) -> PaperGateChallengeRecord:
    if len(row) != len(_CHALLENGE_COLUMN_NAMES):
        raise PaperGateAuthorityUnavailable("stored paper Gate row shape is invalid")
    values = dict(zip(_CHALLENGE_COLUMN_NAMES, row, strict=True))
    return PaperGateChallengeRecord(
        gate_id=str(values["gate_id"]),
        owner_user_id=UUID(str(values["owner_user_id"])),
        workspace_id=str(values["workspace_id"]),
        gate_kind=str(values["gate_kind"]),  # type: ignore[arg-type]
        status=str(values["status"]),
        task_ref=str(values["task_ref"]),
        expected_task_version=int(values["expected_task_version"]),
        attempt_ref=_optional_row_text(values["attempt_ref"]),
        hqa_gate_ref=str(values["hqa_gate_ref"]),
        platform_session_id=str(values["platform_session_id"]),
        hermes_session_id=str(values["hermes_session_id"]),
        ...
        subject_command_id=(
            ...
            else UUID(str(values["subject_command_id"]))
        ),
        subject_hermes_run_id=_optional_row_text(values["subject_hermes_run_id"]),
        subject_run_attestation_ref=_optional_row_text(values["subject_run_attestation_ref"]),
        subject_run_attestation_digest=_optional_row_text(
            values["subject_run_attestation_digest"],
            strip=True,
        ),
        final_backtest_provider=_optional_row_text(values["final_backtest_provider"]),
        final_backtest_receipt_digest=_optional_row_text(
            values["final_backtest_receipt_digest"],
            strip=True,
        ),
        final_backtest_config_ref=_optional_row_text(values["final_backtest_config_ref"]),
        final_backtest_config_digest=_optional_row_text(
            values["final_backtest_config_digest"],
            strip=True,
        ),
        final_backtest_summary_ref=_optional_row_text(values["final_backtest_summary_ref"]),
        final_backtest_summary_digest=_optional_row_text(
            values["final_backtest_summary_digest"],
            strip=True,
        ),
        final_backtest_report_ref=_optional_row_text(values["final_backtest_report_ref"]),
        final_backtest_report_digest=_optional_row_text(
            values["final_backtest_report_digest"],
            strip=True,
        ),
        parent_gate_id=_optional_row_text(values["parent_gate_id"]),
        source_file_ref=_optional_row_text(values["source_file_ref"]),
        universe=_optional_row_text(values["universe"]),
        reviewed_source_sha256=_optional_row_text(
            values["reviewed_source_sha256"],
            strip=True,
        ),
        gate1_confirmation_id=_optional_row_text(values["gate1_confirmation_id"]),
        candidate_id=_optional_row_text(values["candidate_id"]),
        expected_digest=_optional_row_text(values["expected_digest"], strip=True),
        expected_status=_optional_row_text(values["expected_status"]),
        final_backtest_receipt_id=_optional_row_text(values["final_backtest_receipt_id"]),
        base_commit=_optional_row_text(values["base_commit"], strip=True),
        hqa_receipt_ref=_optional_row_text(values["hqa_receipt_ref"]),
        hqa_receipt_digest=_optional_row_text(
            values["hqa_receipt_digest"],
            strip=True,
        ),
        promotion_id=_optional_row_text(values["promotion_id"]),
        worktree_ref=_optional_row_text(values["worktree_ref"]),
        patch_ref=_optional_row_text(values["patch_ref"]),
        manifest_ref=_optional_row_text(values["manifest_ref"]),
        decided_action_kind=_optional_row_text(values["decided_action_kind"]),
        decided_client_action_id=_optional_row_text(values["decided_client_action_id"]),
        decided_action_digest=_optional_row_text(
            values["decided_action_digest"],
            strip=True,
        ),
        decided_at=values["decided_at"],  # type: ignore[arg-type]
...
def _completion_from_row(
    ...
    if len(row) != len(_COMPLETION_COLUMN_NAMES):
        raise PaperGateAuthorityUnavailable("stored paper completion row shape is invalid")
    values = dict(zip(_COMPLETION_COLUMN_NAMES, row, strict=True))
    evidence = values["completion_evidence"]
    ...
    return PaperGateCompletionRecord(
        ...
        task_terminal_outcome=str(values["task_terminal_outcome"]),
        plan_version=int(values["plan_version"]),
        plan_digest=str(values["plan_digest"]).strip(),
        plan_confirmation_note_digest=str(values["plan_confirmation_note_digest"]).strip(),
        attempt_ref=str(values["attempt_ref"]),
        attempt_status=str(values["attempt_status"]),
        attempt_terminal_outcome=str(values["attempt_terminal_outcome"]),
        domain_gate_ref=str(values["domain_gate_ref"]),
        domain_gate_outcome=str(values["domain_gate_outcome"]),
        hqa_run_ref=str(values["hqa_run_ref"]),
        provider_evidence_ref=str(values["provider_evidence_ref"]),
        subject_command_id=UUID(str(values["subject_command_id"])),
        subject_hermes_run_id=str(values["subject_hermes_run_id"]),
        subject_run_attestation_ref=str(values["subject_run_attestation_ref"]),
        subject_run_attestation_digest=str(values["subject_run_attestation_digest"]).strip(),
        final_backtest_provider=str(values["final_backtest_provider"]),
        final_backtest_receipt_digest=str(values["final_backtest_receipt_digest"]).strip(),
        final_backtest_config_ref=str(values["final_backtest_config_ref"]),
        final_backtest_config_digest=str(values["final_backtest_config_digest"]).strip(),
        final_backtest_summary_ref=str(values["final_backtest_summary_ref"]),
        final_backtest_summary_digest=str(values["final_backtest_summary_digest"]).strip(),
        final_backtest_report_ref=str(values["final_backtest_report_ref"]),
        final_backtest_report_digest=str(values["final_backtest_report_digest"]).strip(),
        promotion_id=str(values["promotion_id"]),
        reviewed_commit=str(values["reviewed_commit"]).strip(),
        candidate_id=str(values["candidate_id"]),
        candidate_digest=str(values["candidate_digest"]).strip(),
        final_backtest_receipt_id=str(values["final_backtest_receipt_id"]),
        base_commit=str(values["base_commit"]).strip(),
        attempt_completion_operation_id=str(values["attempt_completion_operation_id"]),
        attempt_completion_event_id=str(values["attempt_completion_event_id"]),
        task_completion_operation_id=str(values["task_completion_operation_id"]),
        task_completion_event_id=str(values["task_completion_event_id"]),
        workflow_audit_status=str(values["workflow_audit_status"]),
        workflow_audit_ref=str(values["workflow_audit_ref"]),
        workflow_audit_digest=str(values["workflow_audit_digest"]).strip(),
        hqa_completion_receipt_ref=str(values["hqa_completion_receipt_ref"]),
        hqa_completion_receipt_digest=str(values["hqa_completion_receipt_digest"]).strip(),
...
def paper_gate_schema_is_ready_on_connection(
    ...
    if any(
        set(observed_columns[table]) != set(expected)
        or len(observed_columns[table]) != len(expected)
...
def paper_gate_runtime_security_is_ready_on_connection(
    conn: psycopg.Connection,
) -> bool:
    """Require the active connection itself to be the constrained runtime.

    Schema/global-role readiness is insufficient here: a service accidentally
    configured with the migrator or a superuser URL could otherwise bypass
    FORCE RLS while every catalog-level readiness check still returned true.
    """
...
def paper_gate_runtime_security_ready(settings: Settings) -> bool:
    try:
        database = get_database(settings)
        if database is None:
            return False
        with database.connect() as conn:
            return paper_gate_runtime_security_is_ready_on_connection(conn)
    except (DatabaseUnavailable, psycopg.Error):
        return False
...
class PaperGateAuthority:
    ...
    def _require_schema(conn: psycopg.Connection) -> None:
        if not paper_gate_runtime_security_is_ready_on_connection(conn):
            raise PaperGateAuthorityUnavailable("paper gate runtime security is not ready")
            ...
            with self._database().connect() as conn, conn.transaction():
                ...
                row = conn.execute(
                    ...
                    (
                        ...
                        values.subject_run_attestation_digest,
                        values.final_backtest_provider,
                        values.final_backtest_receipt_digest,
                        values.final_backtest_config_ref,
                        values.final_backtest_config_digest,
                        values.final_backtest_summary_ref,
                        values.final_backtest_summary_digest,
                        values.final_backtest_report_ref,
                        values.final_backtest_report_digest,
    ...
    def _validate_registration(
        ...
        provider_evidence_ref = _optional_ref(
            request.provider_evidence_ref,
            "provider_evidence_ref",
        ...
        subject_hermes_run_id = (
            None
            if request.subject_hermes_run_id is None
            else _identifier(
                request.subject_hermes_run_id,
                "subject_hermes_run_id",
            )
        )
        subject_attestation_ref = _optional_ref(
            request.subject_run_attestation_ref,
            "subject_run_attestation_ref",
            _PAPER_RUN_ATTESTATION_RE,
        )
        subject_attestation_digest = (
            None
            if request.subject_run_attestation_digest is None
            else _digest(
                request.subject_run_attestation_digest,
                "subject_run_attestation_digest",
            )
        )
        final_provider = request.final_backtest_provider
        if final_provider is not None and final_provider != "futu":
            raise PaperGateAuthorityValidationError("final_backtest_provider must be futu")
        final_receipt_digest = (
            None
            if request.final_backtest_receipt_digest is None
            else _digest(
                request.final_backtest_receipt_digest,
                "final_backtest_receipt_digest",
            )
        )
        final_config_ref = _optional_absolute_ref(
            request.final_backtest_config_ref,
            "final_backtest_config_ref",
        )
        final_config_digest = (
            None
            if request.final_backtest_config_digest is None
            else _digest(
                request.final_backtest_config_digest,
                "final_backtest_config_digest",
            )
        )
        final_summary_ref = _optional_absolute_ref(
            request.final_backtest_summary_ref,
            "final_backtest_summary_ref",
        )
        final_summary_digest = (
            None
            if request.final_backtest_summary_digest is None
            else _digest(
                request.final_backtest_summary_digest,
                "final_backtest_summary_digest",
            )
        )
        final_report_ref = _optional_absolute_ref(
            request.final_backtest_report_ref,
            "final_backtest_report_ref",
        )
        final_report_digest = (
            None
            if request.final_backtest_report_digest is None
            else _digest(
                request.final_backtest_report_digest,
                "final_backtest_report_digest",
            )
        )
        terminal_fields = (
        ...
        if request.gate_kind == "gate1":
            ...
            valid = (
                ...
                and all(value is not None for value in terminal_fields)
                and subject_command_id != command_id
                and subject_hermes_run_id != run_id
                and hqa_run_ref == f"run:{subject_hermes_run_id}"
                and subject_attestation_ref == f"paper-run-attestation:{subject_attestation_digest}"
        ...
        return replace(
            ...
            final_backtest_provider=final_provider,
            final_backtest_receipt_digest=final_receipt_digest,
            final_backtest_config_ref=final_config_ref,
            final_backtest_config_digest=final_config_digest,
            final_backtest_summary_ref=final_summary_ref,
            final_backtest_summary_digest=final_summary_digest,
            final_backtest_report_ref=final_report_ref,
            final_backtest_report_digest=final_report_digest,
    ...
    def _same_registration(
        ...
        return (
            ...
            and record.subject_run_attestation_ref == request.subject_run_attestation_ref
            and record.subject_run_attestation_digest == request.subject_run_attestation_digest
            and record.final_backtest_provider == request.final_backtest_provider
            and record.final_backtest_receipt_digest == request.final_backtest_receipt_digest
            and record.final_backtest_config_ref == request.final_backtest_config_ref
            and record.final_backtest_config_digest == request.final_backtest_config_digest
            and record.final_backtest_summary_ref == request.final_backtest_summary_ref
            and record.final_backtest_summary_digest == request.final_backtest_summary_digest
            and record.final_backtest_report_ref == request.final_backtest_report_ref
            and record.final_backtest_report_digest == request.final_backtest_report_digest
    ...
    def _validated_completion(
        ...
        plan_version = evidence.get("plan_version")
        if (
            isinstance(plan_version, bool)
            or not isinstance(plan_version, int)
            or not 1 <= plan_version <= 2**31 - 1
        ):
            raise PaperGateAuthorityValidationError("completion plan_version must be positive")
        exact_patterns: tuple[
            ...
            ("subject_run_attestation_ref", _PAPER_RUN_ATTESTATION_RE),
        ...
        subject_run_id = _identifier(
            ...
            "completion_evidence subject_hermes_run_id",
        )
        if (
            evidence["hqa_run_ref"] != f"run:{subject_run_id}"
            or evidence["subject_run_attestation_ref"]
            != (f"paper-run-attestation:{evidence['subject_run_attestation_digest']}")
            or evidence["provider_evidence_ref"]
            != (f"provider-evidence:paper-run-{evidence['subject_run_attestation_digest']}")
            or evidence.get("final_backtest_provider") != "futu"
        ):
            raise PaperGateAuthorityValidationError(
                "completion subject/Futu evidence binding is invalid"
            )
        evidence["subject_command_id"] = subject_command_id
        for field in (
            "final_backtest_config_ref",
            "final_backtest_summary_ref",
            "final_backtest_report_ref",
        ):
            _optional_absolute_ref(evidence.get(field), f"completion_evidence {field}")
        ...
        try:
            with self._database().connect() as conn, conn.transaction():
                ...
                if (
                    ...
                    or gate.provider_evidence_ref != evidence["provider_evidence_ref"]
                    or (None if gate.subject_command_id is None else str(gate.subject_command_id))
                    != evidence["subject_command_id"]
                    or gate.subject_hermes_run_id != evidence["subject_hermes_run_id"]
                    or gate.subject_run_attestation_ref != evidence["subject_run_attestation_ref"]
                    or gate.subject_run_attestation_digest
                    != evidence["subject_run_attestation_digest"]
                    or gate.final_backtest_provider != evidence["final_backtest_provider"]
                    or gate.final_backtest_receipt_digest
                    != evidence["final_backtest_receipt_digest"]
                    or gate.final_backtest_config_ref != evidence["final_backtest_config_ref"]
                    or gate.final_backtest_config_digest != evidence["final_backtest_config_digest"]
                    or gate.final_backtest_summary_ref != evidence["final_backtest_summary_ref"]
                    or gate.final_backtest_summary_digest
                    != evidence["final_backtest_summary_digest"]
                    or gate.final_backtest_report_ref != evidence["final_backtest_report_ref"]
                    or gate.final_backtest_report_digest != evidence["final_backtest_report_digest"]
        ...
        try:
            ...
            return [
                _gate_public_projection(
                    _record_from_row(tuple(row[: len(_CHALLENGE_COLUMN_NAMES)])),
                    action_state=(
                        None
                        if row[len(_CHALLENGE_COLUMN_NAMES)] is None
                        else str(row[len(_CHALLENGE_COLUMN_NAMES)])
                    ),
                    action_error=(
                        None
    ...
    def get_gate1_source_evidence(
        self,
        *,
        workspace_id: str,
        gate_id: str,
    ) -> dict[str, object]:
        """Return digest-verified source bytes for one owner/workspace Gate 1.

        The HTTP caller never provides a file path or digest.  Both values come
        from the exact durable challenge, and the bounded reader re-hashes the
        no-follow regular file before any bytes are returned.
        """
            ...
            with self._database().connect() as conn:
                ...
                row = conn.execute(
                    ...
                    WHERE owner_user_id = %s
                      ...
                      AND gate_id = %s
                    """,
                    (ROOT_USER_ID, workspace, exact_gate_id),
                ).fetchone()
            if row is None:
                raise PaperGateNotFound()
            record = _record_from_row(row)
            if (
                record.gate_kind != "gate1"
                or record.source_file_ref is None
                or record.reviewed_source_sha256 is None
            ):
                raise PaperGateAuthorityConflict(
                    "paper gate does not contain reviewable Gate 1 source evidence"
                )
            evidence = read_verified_gate1_source(
                source_file_ref=record.source_file_ref,
                reviewed_source_sha256=record.reviewed_source_sha256,
            )
            return evidence.to_public_dict(
                gate_id=record.gate_id,
                workspace_id=record.workspace_id,
            )
        except (PaperGateAuthorityError, PaperGateSourceEvidenceError):
            raise
        except (DatabaseUnavailable, psycopg.Error) as exc:
            raise PaperGateAuthorityUnavailable("paper Gate 1 source observation failed") from exc
...
__all__ = [
    ...
    "paper_gate_runtime_security_ready",
