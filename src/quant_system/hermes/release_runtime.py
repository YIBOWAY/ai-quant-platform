    ...
    *,
    ...
    if name == "hermes_command_approval_exact_cas":
        ...
        for field in (
            ...
            _hex_digest(evidence[field], field=f"{name}.{field}")
        identities = [
        ...
        event_ids = _identity_list(
            evidence["event_ids"],
            field=f"{name}.event_ids",
            minimum=4,
        )
        _utc_timestamp(
            evidence["expected_expires_at"],
            field=f"{name}.expected_expires_at",
        )
        valid_choice = (
            evidence["decision"] == "allow_once"
            and evidence["choice"] == "once"
        ) or (
            evidence["decision"] == "deny"
            and evidence["choice"] == "deny"
        )
        if (
            evidence["route"] != "/hermes"
            or not valid_choice
            or evidence["run_status"] != "succeeded"
            or evidence["waiter_signal_status"] != "confirmed"
            or len(event_ids) != 4
            or identities[2] == identities[3]
        ):
            raise ReleaseRuntimeProbeError(
                "release evidence Hermes approval CAS flow is invalid"
            )
        return
    ...
    if name == "hermes_run_stop_recovery":
            ...
            keys=frozenset(
                {
                    "action_digest",
                    "control_command_id",
                    "idempotent_recovery_proven",
                    "platform_command_id",
                    "post_restart_instance_id",
                    "route",
                    "run_id",
                    "status",
        ...
        _hex_digest(evidence["action_digest"], field=f"{name}.action_digest")
        identities = {
            field: _bounded_identity(evidence[field], field=f"{name}.{field}")
            for field in (
                "control_command_id",
                "platform_command_id",
                "post_restart_instance_id",
        ...
        if (
            evidence["route"] != "/hermes"
            or evidence["status"] != "stopped"
            or evidence["terminal_event"] != "run.cancelled"
            or evidence["idempotent_recovery_proven"] is not True
            or identities["control_command_id"]
            == identities["platform_command_id"]
            or identities["stop_requested_event_id"]
            == identities["terminal_event_id"]
        ):
            raise ReleaseRuntimeProbeError(
                "release evidence Hermes Run-stop recovery flow is invalid"
            )
        return
...
def restricted_runtime_security_ready(settings: Settings) -> bool:
    ...
    try:
        ...
        if not release_authority_runtime_security_ready(settings):
            return False
        if not run_control_outcome_runtime_security_ready(settings):
            return False
