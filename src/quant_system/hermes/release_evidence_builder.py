...
def _safe_output_directory(
    ...
    try:
        for component in absolute.parts[1:]:
            ...
            if stat.S_ISLNK(info.st_mode):
                raise EvidenceBuildError("evidence output directory must not use a symlink")
        info = os.stat(absolute, follow_symlinks=False)
    except FileNotFoundError as exc:
        raise EvidenceBuildError("evidence output directory must already exist") from exc
    except OSError as exc:
        raise EvidenceBuildError("evidence output directory is unavailable") from exc
    if not stat.S_ISDIR(info.st_mode):
        raise EvidenceBuildError("evidence output path is not a directory")
    ...
    if os.name != "nt" and stat.S_IMODE(info.st_mode) & 0o022:
        raise EvidenceBuildError("evidence output directory must not be group/world writable")
...
def _seal_bundle(
    *,
    ...
    try:
        dir_fd = os.open(output_dir, flags)
    except OSError as exc:
        raise EvidenceBuildError("evidence output directory became unavailable") from exc
    created = False
    try:
...
def _read_temporary_junit(path: Path) -> bytes:
    flags = os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) | getattr(os, "O_NOFOLLOW", 0)
    try:
...
def run_test_suite(
    *,
    ...
    try:
        temporary_fd = os.open(temporary_path, temporary_flags, 0o600)
    except OSError as exc:
    ...
    try:
        ...
        junit = _read_temporary_junit(temporary_path)
        try:
...
def _flow_documents(
    ...
    for name in _FLOW_NAMES:
        ...
        artifact = {
            ...
            "evidence": _flow_projection(name, flows[name]),
            "kind": "real_flow",
            "name": name,
            "passed": True,
